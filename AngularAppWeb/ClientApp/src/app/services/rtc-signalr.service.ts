...
export class RtcSignalRService {
  ...
  async startConnection() {
    ...
    this._hubConnection.on(
      'callToUserList',
      async (roomName: string, users: IUser[]) => {
    ...
    this._hubConnection.on(
      'receiveSignal',
      async (user: IUser, signal: string) => {
  ...
  private async updateUserList(users: IUser[]): Promise<void> {
    ...
    users.forEach(async (user) => {
      ...
      if (
        ...
        connection.streamSub.getValue() === undefined
      ) {
  ...
  private async receivedVideoOffer(
    partnerClientId: string,
    sdp: RTCSessionDescription
  ) {
    ...
    try {
      ...
      if (!senders || senders.length === 0) {
        ...
        localStream
          .getTracks()
          .forEach((track) =>
            connection.rtcConnection.addTrack(track, localStream)
          );
      }
  ...
  private async receivedVideoAnswer(
    partnerClientId: string,
    sdp: RTCSessionDescription
  ) {
  ...
  private async createConnection(
    ...
    connection.onicegatheringstatechange = () => {
      console.log(
        '*** ICE gathering state changed to: ' + connection.iceGatheringState
      );
    };
    connection.onsignalingstatechange = (event) => {
      console.log(
        '*** WebRTC signaling state changed to: ' + connection.signalingState
      );
